...
!    to lat-lon grid at 0.04 degree, using the ipolates lib. 
!
...
!  $Id: 4km-0.04.F90,v 1.1 2012/03/02 22:26:39 ytian Exp ytian $
!  Usage: 
!  ./4km-0.04 inputfile outputfile 
!-----------------------------------------------------------------
      implicit none

      integer, parameter :: icols=1121, irows=881   !** input grid
      integer, parameter :: MI = icols * irows       !** map to 1D 
      integer, parameter :: ncols=1375, nrows=600  !** 0.25 
      integer, parameter :: NLDAS = ncols * nrows     !** map to 1D 
      real, parameter :: xres = 0.04          !** 5km res
      real, parameter :: yres = 0.04
...
!*** grib stuff 
      ...
      End If
       ...
       write(*, *)"Warning: this program should be run on discover nodes." 
       write(*, *)"     Do not run it on Dali, which will cause the program to crash!" 
      call getarg(1, ifile)
      call getarg(2, ofile)

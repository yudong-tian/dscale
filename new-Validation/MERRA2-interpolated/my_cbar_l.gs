...
*
*  Written MC, LSCE, Mars 2000, Modified CBAR_LINE script
*  Modify by B.-J. Tsuang, Feb 2004, add -h option
*
function cbarline (args)
*
****** run script to create a legend when grads choosed line style color and symbols
******    cbar_l -x X -y Y -n number -t text -h -p
******
******  cbar_l parameters: 
...
******     h        if set, draw legend horizontally (default vertically).
...
drawHorizontal=0
***********************   USAGE   **************************************************
usage1="usage: cbar_l -x X -y Y -n # -t text -h -p"
...
usage6="-h          if set, draw legend horizontally (default vertically)."
usage7="-p          if set, user can click on graphic to set legend."
usage8=" "
usage9="If one keyword misses a value, uses default for the remaining"
usage10="To include zeros, zeros has to be double quoted"
usage11='Ex.: cbar_l -t "line1" "line2" "line3" -p'
...
while (f!='')
  ...
  if (opt='h' | opt='H'); drawHorizontal=1; endif;
...
marks.1=2;marks.2=2;marks.3=4;marks.4=5;marks.5=1;marks.6=2;marks.7=3;marks.8=4;marks.9=5;marks.10=1;
...
while (o<maximum)
  ...
  if (drawHorizontal=1)
    if (o=1)
      ...
      xl=xr+0.1+0.12*(wrdlen(texts.old)+2)
    endif
    xr=xl+xwid+0.5
  else
    y=y-ywid
